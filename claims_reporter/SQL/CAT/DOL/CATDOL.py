...
main('whan@markelcorp.com', '7ujm*IK<')
def main(email_user, email_pwd, sendmail=False):
    ...
    path = r'\\MKLFILE\CLAIMS\corpfs06-filedrop\ClaimsReporting\Projects\CAT-Automate\CAT_files\DOLCATReportTEMPLATEtest.xlsm'
    macro = 'Dump_Final.Dump_Final'
    saveas = r'\\MKLFILE\CLAIMS\corpfs06-filedrop\ClaimsReporting\Ad Hoc Reporting\{year}\CAT\Reports\{today} - DOL CAT Master Report.xlsx'.format(year=year, today=today)
    ...
    attach = [r'\\MKLFILE\CLAIMS\corpfs06-filedrop\ClaimsReporting\Ad Hoc Reporting\{year}\CAT\Reports\{today} - DOL CAT Master Report.xlsx'.format(year=year, today=today)]
...
#    MPL_SP(ClaimsCNN)
    ...
    EclipseBackup = EclipseBackup[(EclipseBackup['Accident Date'] >= '2019-05-18') & (EclipseBackup['Accident Date'] <= '2019-05-24')]   # Date of Loss
...
def queryDB(filename,CNN):
    with open(r'\\Mklfile\claims\corpfs06-filedrop\ClaimsReporting\Projects\CAT-Automate\SQL\forPython\DOL\%s' % filename) as f:
        query=f.read()
...
def ERMSbase():
    with open(r'\\Mklfile\claims\corpfs06-filedrop\ClaimsReporting\Projects\CAT-Automate\SQL\forPython\DOL\ERMS\ERMStemp.sql') as t:
        temp = t.read()
    return temp

def ERMSCAT():
    with open(r'\\Mklfile\claims\corpfs06-filedrop\ClaimsReporting\Projects\CAT-Automate\SQL\forPython\DOL\ERMS\ERMSfinal.sql') as f:
        final = f.read()
    return final
